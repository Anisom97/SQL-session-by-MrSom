...
;
 ...
 (SELECT C.AMP_ID
		...
		FROM CUST_360_INFO_V1 C
		LEFT JOIN CUSTSEGROLL_V1 CR
		ON C.AMP_ID=CR.AMPERITY_ID) T
		WHERE T.CUSTOMER_SEG IS NULL
		;
...
CUSTSEG AS (SELECT A.AMPERITY_ID
					, COALESCE (CR.SEGMENT,C.CUSTOMER_SEGMENTS) AS CUSTOMER_SEG
...
SELECT T.AMPERITY_ID
		...
		--ON CONVERT(DATE, CA.DAY_NAME) = CONVERT(DATE, T.ORDER_DATETIME)

		GROUP BY T.AMPERITY_ID, CUSTOMER_SEG, TIER_NAME;